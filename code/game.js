...
var actorChars = {
  "@": Player,
  "o": Barrel, // A barrel will wobble up and down
  "=": Acid, "|": Acid, "v": Acid  
};
...
function Player(pos) {
  this.pos = pos.plus(new Vector(0, -1.1));
  this.size = new Vector(2.3, 1.9);
...
function Acid(pos, ch) {
  ...
  if (ch == "=") {
    ...
    // Acid Drip. Repeat back to this pos.
...
var wobbleSpeed = 15, wobbleDist = 0.05;

Barrel.prototype.act = function(step) {
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "acid" && this.status == null) {
    ...
    // If there aren't any Barrels left, player wins
    if (!this.actors.some(function(actor) {
           return actor.type == "barrel";
         })) {
      this.status = "won";
      this.size = Vector(3, 3);
      this.finishDelay = 3;
    }
  }
};
